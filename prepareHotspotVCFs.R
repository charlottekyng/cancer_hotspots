...
muts$hotindel <- apply(muts, 1, function(x){
  if (!grepl("inframe", x["EFFECT"])) {
    "."
  } else if (length(as.character(unlist(indel[which(indel$CHROM == x["CHROM"] & indel$GENE == x["GENE"] & indel$aa_start <= x["AA.pos"] & indel$aa_end >= x["AA.pos.end"]), which(colnames(indel) == "hotspot_type")]))) == 0) {
    "."
  } else {
    as.character(unlist(indel[which(indel$CHROM == x["CHROM"] & indel$GENE == x["GENE"] & indel$aa_start <= x["AA.pos"] & indel$aa_end >= x["AA.pos.end"]), which(colnames(indel) == "hotspot_type")]))
  }
})
